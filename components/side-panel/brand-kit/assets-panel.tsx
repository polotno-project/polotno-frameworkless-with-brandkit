...
import { Edit, Trash, Upload } from '@blueprintjs/icons';
...
export const AssetsPanel = observer(
  ({ store, columns = 2 }: AssetsPanelProps) => {
    ...
    const {
      ...
      defaultQuery: '',
      getAPI: ({ page, query }) =>
        JSON.stringify({ type: 'assets', page, query, pageSize: PAGE_SIZE }),
    ...
    const handleAssetSelect = async ({
      ...
      droppedPos,
      targetElement,
    }: HandleAssetSelectParams) => {
      const fileType = asset.url.toLowerCase().includes('.svg')
        ? 'svg'
        : 'image';
    ...
    return (
      <>
        ...
        <BrandKitModal
          ...
          title={
            editingAsset ? t('brandKit.editAsset') : t('brandKit.createAsset')
          }
          width="400px"
        >
