...
export const ColorsPanel = observer(
  ({ store, columns = 3 }: ColorsPanelProps) => {
    const { colors } = useBrandKit();
    const [isModalOpen, setIsModalOpen] = React.useState(false);
    const [editingColor, setEditingColor] = React.useState<Color | null>(null);
    const [formState, setFormState] = React.useState<ColorFormState>(
      defaultColorFormState
    );
    ...
    const {
      ...
      defaultQuery: '',
      getAPI: ({ page, query }) =>
        JSON.stringify({ type: 'colors', page, query, pageSize: PAGE_SIZE }),
    ...
    const handleCreateColor = () => {
      ...
      if (
        firstElement &&
        'fill' in firstElement &&
        typeof firstElement.fill === 'string'
      ) {
        color = colorToHex(firstElement.fill);
    ...
    return (
      <>
        ...
        <BrandKitModal
          ...
          title={
            editingColor ? t('brandKit.editColor') : t('brandKit.createColor')
          }
        >
          ...
          <BrandKitModalActions
            ...
            saveDisabled={
              !formState.name || !formState.hex || !isValidHex(formState.hex)
            }
          />
        </BrandKitModal>
