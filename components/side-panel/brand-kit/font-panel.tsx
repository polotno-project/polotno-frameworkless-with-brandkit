...
import { CaretDown, Search, Edit, Trash } from '@blueprintjs/icons';
...
const FontMenu = ({
  ...
  const filteredFonts = fonts.filter((font: string) => {
                ...
                return (
                  <div style={style}>
                    <FontItem
                      ...
                      isCustom={
                        !!(
                          store.fonts.find((f: any) => f.fontFamily === item) ||
                          globalFonts.find((f: any) => f.fontFamily === item)
                        )
                      }
                    />
                  </div>
...
const FontPreview = styled(ItemPreview)<FontPreviewProps>`
  display: flex;
  align-items: center;
  justify-content: center;
  background-color: #f5f8fa;
  font-family: ${(props: FontPreviewProps) => props.fontFamily};
  font-size: ${(props: FontPreviewProps) => Math.min(props.fontSize, 24)}px;
  font-weight: ${(props: FontPreviewProps) => (props.bold ? 'bold' : 'normal')};
  font-style: ${(props: FontPreviewProps) =>
    props.italic ? 'italic' : 'normal'};
  color: #2c3e50;
  text-align: center;
  padding: 8px;
  line-height: 1.2;
  text-decoration: ${(props: FontPreviewProps) =>
...
const SelectFontFamily = observer(
  ({
    value,
    store,
    onFontSelect,
...
export const FontPanel = observer(({ store, columns = 3 }: FontPanelProps) => {
  ...
  const [formState, setFormState] =
    React.useState<FontFormState>(defaultFontFormState);
  ...
  const {
    ...
    defaultQuery: '',
    getAPI: ({ page, query }) =>
      JSON.stringify({ type: 'typography', page, query, pageSize: PAGE_SIZE }),
  ...
  const handleCreateFont = () => {
    if (
      ...
      store.selectedElements[0].type === 'text'
    ) {
      ...
      >
        ...
        <FontFormGroup label={t('brandKit.fontFamily')} labelFor="font-family">
          <SelectFontFamily
            ...
            onFontSelect={(fontFamily) =>
              setFormState({ ...formState, fontFamily })
            }
          />
        </FontFormGroup>
        ...
        <div style={{ display: 'flex', gap: '15px' }}>
          ...
          >
            <NumericInput
              ...
              onValueChange={(value) =>
                setFormState({ ...formState, fontSize: value || 16 })
              }
              min={8}
              max={200}
              stepSize={1}
        ...
        <FontFormGroup label={t('brandKit.lineHeight')} labelFor="line-height">
          <NumericInput
            ...
            onValueChange={(value) =>
              setFormState({ ...formState, lineHeight: value || 1.2 })
            }
            min={0.5}
            max={3}
            stepSize={0.1}
        ...
        <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
          <Checkbox
            ...
            onChange={(e) =>
              setFormState({ ...formState, bold: e.currentTarget.checked })
            }
          ...
          <Checkbox
            ...
            onChange={(e) =>
              setFormState({ ...formState, italic: e.currentTarget.checked })
            }
          ...
          <Checkbox
            ...
            onChange={(e) =>
              setFormState({ ...formState, underline: e.currentTarget.checked })
            }
          ...
          <Checkbox
            ...
            onChange={(e) =>
              setFormState({
                ...formState,
                strikethrough: e.currentTarget.checked,
              })
            }
        ...
        <div style={{ marginTop: '20px', marginBottom: '20px' }}>
          ...
          <div
            style={{
              ...
              lineHeight: formState.lineHeight,
              textDecoration: `${formState.underline ? 'underline' : ''} ${
                formState.strikethrough ? 'line-through' : ''
              }`.trim(),
              padding: '10px',
